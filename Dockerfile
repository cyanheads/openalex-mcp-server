...
# that are not needed in the final production image. `--omit=peer` keeps the
# framework's optional peer tiers out; imported peers belong in dependencies.
RUN --mount=type=cache,target=/root/.bun/install/cache \
...
RUN mkdir -p /var/log/openalex-mcp-server && chown -R bun:bun /var/log/openalex-mcp-server
...
ENV LOGS_DIR="/var/log/openalex-mcp-server"
